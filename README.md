...
Clifa는 로컬 환경에서 동작하는 **시맨틱 이미지 검색 데스크톱 앱**입니다.  
다국어 CLIP 임베딩과 FAISS 벡터 인덱스를 결합해, 파일명 기반 탐색보다 빠르고 정확하게 원하는 이미지를 찾을 수 있습니다.

## 🎯 프로젝트 개요

- **문제**: 이미지가 많아질수록 폴더/파일명 기반 탐색의 효율이 급격히 떨어짐
- **해결**: 텍스트-이미지 임베딩 검색으로 "설명으로 찾는" 워크플로우 제공
- **핵심 가치**: 로컬 처리 기반 속도/프라이버시, 다국어 검색, 실사용 중심 UI/UX
...
- 설치 후 바로 꺼지는 것처럼 보일 때:
  ...
  - 네트워크 환경(프록시/방화벽)에서 모델 다운로드가 차단되지 않았는지 확인하세요.
  - 설치 중단 후 재실행 시 관리자 권한으로 다시 실행하면 복구되는 경우가 있습니다.
